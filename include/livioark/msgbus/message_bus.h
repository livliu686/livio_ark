...
public:
    ...
    public:
        ...
        {
            auto s    = state_;
            s->sub_id = s->bus.template subscribe<T>(
                s->topic,
    ...
    unsigned dispatcher_count() const
    {
    ...
    const TopicRegistry& registry() const
    {
        return registry_;
    }
    ...
    TopicHandle<T> topic(std::string_view topic, OnDrop&& on_drop)
    {
        ...
        return TopicHandle<T>(this,
                              tid,
                              registry_.to_string(tid),
    ...
    bool enqueueMessage(MessagePtr mptr)
    {
        ...
        switch (policy_)
        {
            ...
            case FullPolicy::DropOldest:
            {
                ...
                if (!queue_.try_enqueue(mptr))
                {  // copy (keep mptr valid for retry)
                    MessagePtr oldest;
